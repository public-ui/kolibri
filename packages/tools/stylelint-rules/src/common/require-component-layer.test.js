...
describe('kolibri/require-component-layer', () => {
	...
	it('rejects any @include outside @layer (themes, strict=true)', async () => {
		const result = await stylelint.lint({
	...
	it('rejects @include kol-icon-theme outside @layer (themes, strict=true)', async () => {
		const result = await stylelint.lint({
			code: '@use "../mixins/icon" as *;\n@include kol-icon-theme;\n@layer kol-theme-component { .button { color: red; } }',
			config: config('kol-theme-component', '/src/components/'),
			codeFilename: themesComponentPath,
		});
		assert.ok(result.results[0].warnings.length > 0);
	...
	it('accepts @include inside @layer (themes, strict=true)', async () => {
		const result = await stylelint.lint({
			code: '@layer kol-theme-component { @include kol-icon-theme; @include some-unknown-mixin; .button { color: red; } }',
			config: config('kol-theme-component', '/src/components/'),
			codeFilename: themesComponentPath,
		});
		assert.equal(result.results[0].warnings.length, 0);
