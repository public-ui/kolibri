...
if (!fs.existsSync(distPath)) {
	throw new Error(
		'Cannot find the hydration bundle. Run "pnpm --filter @public-ui/components build" before executing the hydration adapter tests.'
	);
}
...
const snapshotHtml = '<kol-button _label="Snapshot Button" _variant="primary"></kol-button><kol-alert _heading="Snapshot alert" _variant="info">Snapshot content</kol-alert>';
const snapshotDocument =
	'<!DOCTYPE html><html lang="de" dir="ltr"><head><meta charset="utf-8"><title>Hydration Snapshot</title></head><body>' +
	snapshotHtml +
	'</body></html>';
...
describe('Hydration adapter snapshots', function () {
	...
	it('hydrates a full document string', async () => {
		const result = await hydrateDocument(snapshotDocument, hydrateOptions);
