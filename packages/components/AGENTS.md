...
- **Controller layer** – encapsulates state transitions, validation orchestration, and exposes render props via `getProp(key)`.
...
5. Delegate rendering to the controller's `getProp(key)` method for type-safe, single-value prop access.
