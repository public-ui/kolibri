			...
			<li class="kol-breadcrumb__list-element" key={index}>
				{index === lastIndex ? (
					<span class="kol-breadcrumb__list-element-span" aria-current="page">
						{link._hideLabel ? (
							<KolIconTag class="kol-breadcrumb__icon" _label={link._label} _icons={typeof link._icons === 'string' ? link._icons : 'kolicon-kolibri'} />
						) : (
							<>{link._label}</>
						)}
					</span>
			...
			<nav class="kol-breadcrumb" aria-label={this.state._label}>
				<ul class="kol-breadcrumb__list">
					{this.state._links.length === 0 && (
						<li>
							<KolIconTag class="kol-breadcrumb_icon" _label="" _icons="kolicon-kolibri" />…
						</li>
					)}
