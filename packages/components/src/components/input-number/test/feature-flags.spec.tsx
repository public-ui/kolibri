...
const render = (props: { _disabled?: boolean; _readOnly?: boolean } = {}) =>
	newSpecPage({
		components: [KolInputNumber],
		template: () => <kol-input-number _label="Number" {...props} />,
	});
...
describe('KolInputNumber – inputNumberButtons feature flag', () => {
	...
	it('keeps step buttons hidden when disabled, even if the flag is "show"', async () => {
		getFeatureFlagMock.mockReturnValue('show');
		const page = await render({ _disabled: true });
		await page.waitForChanges();
	...
	it('keeps step buttons hidden when read-only, even if the flag is "show"', async () => {
		getFeatureFlagMock.mockReturnValue('show');
		const page = await render({ _readOnly: true });
		await page.waitForChanges();
