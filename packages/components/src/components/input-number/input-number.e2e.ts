...
test.describe(COMPONENT_NAME, () => {
	...
	test.describe('Callbacks and Events', () => {
		test('should call onFocus callback and emit focus event when input receives focus', async ({ page }) => {
			await setContentWithRetry(page, `<${COMPONENT_NAME} _label="Number"></${COMPONENT_NAME}>`);
			const component = page.locator(COMPONENT_NAME);
			const input = page.locator('input');
			await component.evaluate((element: HTMLKolInputNumberElement) => {
		...
		test('should call onBlur callback and emit blur event when input loses focus', async ({ page }) => {
			await setContentWithRetry(page, `<${COMPONENT_NAME} _label="Number"></${COMPONENT_NAME}><button id="next">Next</button>`);
			const component = page.locator(COMPONENT_NAME);
			const input = page.locator('input');
		...
		test('should call onInput callback and emit input event with value when value changes', async ({ page }) => {
			...
			await page.waitForChanges();
			expect(await page.evaluate(() => (window as unknown as Record<string, unknown>).inputValue)).toBe(Number(TEST_VALUE));
			expect(await page.evaluate(() => (window as unknown as Record<string, unknown>).inputDetail)).toBe(Number(TEST_VALUE));
		});
		...
		test('should call onChange callback and emit change event with value when input is blurred', async ({ page }) => {
			...
			await nextButton.focus();
			await page.waitForChanges();
			expect(await page.evaluate(() => (window as unknown as Record<string, unknown>).changeValue)).toBe(Number(TEST_VALUE));
			expect(await page.evaluate(() => (window as unknown as Record<string, unknown>).changeDetail)).toBe(Number(TEST_VALUE));
		});
	});
	...
	testInputBehaviorContract<HTMLKolInputNumberElement>({
		componentName: COMPONENT_NAME,
		fillAction: async (input) => {
			await input.fill(TEST_VALUE);
		},
		inputSelector: 'input.kol-input[type="number"]',
		pinned: {
			edit: [
				kolEvent('focus'),
				callback('focus'),
				nativeEvent('focus'),
				kolEvent('input', 10.23),
				callback('input', 10.23),
				kolEvent('change', 10.23),
				callback('change', 10.23),
				kolEvent('blur'),
				callback('blur'),
				nativeEvent('blur'),
			],
