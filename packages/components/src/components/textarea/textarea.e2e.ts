...
test.describe(COMPONENT_NAME, () => {
	...
	test.describe('Callbacks and Events', () => {
		test('should call onFocus callback and emit focus event when textarea receives focus', async ({ page }) => {
			...
			await component.evaluate((element: HTMLKolTextareaElement) => {
		...
		test('should call onBlur callback and emit blur event when textarea loses focus', async ({ page }) => {
		...
		test('should call onClick callback and emit click event when textarea is clicked', async ({ page }) => {
			await setContentWithRetry(page, `<${COMPONENT_NAME} _label="Input"></${COMPONENT_NAME}>`);
			const component = page.locator(COMPONENT_NAME);
			const input = page.locator('textarea');
		...
		test('should call onInput callback and emit input event with value when textarea value changes', async ({ page }) => {
			await setContentWithRetry(page, `<${COMPONENT_NAME} _label="Input"></${COMPONENT_NAME}>`);
			const component = page.locator(COMPONENT_NAME);
			const input = page.locator('textarea');
		...
		test('should call onChange callback and emit change event with value when textarea is blurred after change', async ({ page }) => {
			await setContentWithRetry(page, `<${COMPONENT_NAME} _label="Input"></${COMPONENT_NAME}><button id="next">Next</button>`);
			const component = page.locator(COMPONENT_NAME);
			const input = page.locator('textarea');
			const nextButton = page.locator('#next');
	...
	testInputBehaviorContract<HTMLKolTextareaElement>({
		componentName: COMPONENT_NAME,
		fillAction: async (input) => {
			await input.fill(TEST_VALUE);
		},
		inputSelector: 'textarea.kol-textarea',
		pinned: {
			edit: [
				kolEvent('focus'),
				callback('focus'),
