...
**Spin Controller: `watchVariant` parameter typed as `unknown`** _(umgesetzt)_
...
**Meter: `getMeterData()` Dual-Access-Pattern** _(Needs Deeper Look)_
...
**Meter API: manuelles `interface MeterApi extends ComponentApi` statt `ApiFromConfig`** _(Needs Deeper Look)_
