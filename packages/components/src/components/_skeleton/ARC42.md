...
| **Props Pattern**                | Functional components exclusively receive Props that contain either normalized/validated external data or internal component state. Props must always be initialized.                                                                                                                                                                                                                                                                                                                                                    |     | **FC-First Composition** | Web component `render()` methods compose exclusively via Functional Components. KoliBri web component tags (`<kol-*>`) must never appear inside another web component's `render()`. Required controller behaviour from replaced web components must be migrated into the enclosing component's controller. |     | **Shadow DOM First** | All web components use `shadow: true`. Components that should not use Shadow DOM are implemented as Functional Components instead. |
...
13. **No KoliBri web component tags in `render()` — use Functional Components**
    ...
    - _Pragmatic exception_: Components that render a dynamic, data-driven list of links (e.g. `SkipNav`, `Breadcrumb`, `Nav`) may continue to use `<kol-link>` tags until a standardized **Controller-Array pattern** is established for managing N controller instances from external data. This exception is intentionally narrow: it applies only when the link set is fully dynamic and the parent component does not need to intercept link-level events or refs. Fixed-count compositions (one known link per component) must always use FC + Controller.
