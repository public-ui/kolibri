...
11. **ARIA ID uniqueness via `createUniqeId()`**
    - _Pattern_: Any `id` attribute that is referenced by an ARIA relation attribute (`aria-controls`, `aria-labelledby`, `aria-describedby`, `aria-owns`) must be unique per component instance and readable before the nonce. Declare it as a private readonly field: `private readonly popoverId = createUniqeId('popover')`. Use `createUniqeId()` from `utils/dev.utils` (appends a random hex string in production, `'nonce'` in test mode for stable snapshots).
