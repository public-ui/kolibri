...
/**
 * Pinned public API of `kol-input-checkbox` (22 props, 3 methods),
 ...
 */
const KOL_INPUT_CHECKBOX_PUBLIC_API: PublicApiContract = {
	click: {
		kind: 'method',
		type: '',
		required: false,
	...
	_checked: {
		...
		required: false,
		default: 'false',
		doc: 'Defines whether the checkbox is checked or not. Can be read and written. @TODO: Change type back to `CheckedPropType` after Stencil#4663 has been resolved.',
	},
	_disabled: {
		kind: 'prop',
		type: 'boolean',
	...
	_icons: {
		kind: 'prop',
		type: 'Stringified<InputCheckboxIconsProp>',
		required: false,
	...
	_indeterminate: {
		...
		type: 'boolean',
		required: false,
		doc: 'Puts the checkbox in the indeterminate state, does not change the value of _checked. @TODO: Change type back to `IndeterminatePropType` after Stencil#4663 has been resolved.',
	},
	_infoPopover: {
		kind: 'prop',
	...
	_labelAlign: {
		...
		required: false,
		default: "'right'",
		doc: 'Defines which alignment should be used for presentation.',
	},
	_msg: {
		kind: 'prop',
	...
	_variant: {
		kind: 'prop',
		type: 'InputCheckboxVariantPropType',
		required: false,
		default: "'default'",
