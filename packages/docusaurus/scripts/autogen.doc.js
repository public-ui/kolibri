...
const DOC_FOLDERS = ['docs', '30-components'];
const README_PATHS = filterMdFiles(path.relative(process.cwd(), 'doc'));
...
rimraf([...DOC_FOLDERS, '**/*.md'].join('/'), () => {
	README_PATHS.forEach((readmePath) => {
		...
		mkDirs(folders);
		fs.writeFileSync(`${folders.join('/')}/readme.md`, fs.readFileSync(readmePath, 'utf-8').replace('style="color:red"', 'class="text-red-500"'), 'utf-8');
	});
});
