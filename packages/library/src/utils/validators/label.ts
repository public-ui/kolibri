...
const hasEnoughReadableChars = (str: string, min = 1): boolean => countReadableChars(str) >= min;
...
export const validateAriaLabel = (component: Generic.Element.Component, value?: string): void => {
	watchString(component, '_ariaLabel', value, {
		hooks: {
			afterPatch: (value) => {
				if (typeof value === 'string' && value.length > 0 && hasEnoughReadableChars(value, 3) === false) {
					a11yHint(
...
export const validateLabel = (component: Generic.Element.Component, value?: string): void => {
	watchString(component, '_label', value, {
		hooks: {
			afterPatch: (value) => {
				if (typeof value === 'string' && hasEnoughReadableChars(value, 3) === false) {
