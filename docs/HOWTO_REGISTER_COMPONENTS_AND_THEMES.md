...
## 2b. Enable the document-level color scheme (optional, `@public-ui/theme-default`)

The default theme carries a light and a dark palette. The components follow the operating system out of the box — nothing to configure. Load the bundled document stylesheet when the surrounding page should follow along, or when the application wants an explicit switch:
...
The attribute (or class) works on any element and applies to its whole subtree, nested shadow roots included. Without it the operating system decides.
...
- **The page stays light although the OS is dark** \
  The components follow the operating system on their own; the page around them does not. Load `color-scheme.css` (see step 2b) or set `:root { color-scheme: light dark; }` yourself.
- **A `color-scheme` on `<html>` does not reach the components** \
  Each component declares its own `color-scheme`, which shadows an inherited one. Set `--kolibri-color-scheme: light | dark` next to it.
