...
| 10  | Firefox-UA pinnt `font-weight: 400` auf jedes `<button>` → `inherit` → Log default                                                                                                          | Migration unstyled + default + kern | Text-Fettungs-Diffs direkt erklärt                 |
...
| 15  | Exemption-/Größen-Override-Regeln müssen Wrapper UND inneres Element treffen → Log unstyled, bestätigt input-file-40px (Log default)                                                        | Migration unstyled + default + kern | 44px-Icon- und +4px-Container-Diffs direkt erklärt |
