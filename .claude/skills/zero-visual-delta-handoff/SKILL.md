...
| 34  | Der Playwright-Container erreicht die Registry nur mit dem Agent-Proxy-CA: `/root/.ccr/ca-bundle.crt` in den `docker run` mounten und `NODE_EXTRA_CA_CERTS` + `npm_config_cafile` darauf zeigen lassen, sonst bricht `npm install -g pnpm` mit `SELF_SIGNED_CERT_IN_CHAIN` ab                                                                                                                         | Dialog-Skeleton-Migration, Tabs-Skeleton-Migration | Der einzige Blocker zwischen "kein Docker" und laufender Pipeline     |
...
| 38  | Baselines liegen nicht im Git (Erfahrung #32) — lokal erzeugt man sie **vor der ersten Code-Aenderung** im Docker auf dem unveraenderten Base-Stand (`snapshots-docker.mjs <themes…>` ohne `--check`), dann `--check` auf dem Branch. Der Workspace wird zu Beginn **jedes** Laufs gespiegelt: bis `==> pnpm install` im Log steht, keine Datei anfassen, und waehrend einer Theme-Schleife gar nicht | Tabs-Skeleton-Migration (2026-09-23)               | Kein Base-Worktree-Build und keine CI-Runde fuer den Vergleich noetig |
| 39  | `snapshots-docker.mjs a b c --check` bricht nach dem ersten roten Theme ab (`set -euo pipefail`) — fuer die Abnahme je Theme einen eigenen Aufruf in einer Schleife fahren, sonst bleiben die restlichen Themes ungeprueft                                                                                                                                                                            | Tabs-Skeleton-Migration (2026-09-23)               | Ein halber Abnahmelauf, der wie ein ganzer aussieht                   |
| 40  | `axe-snapshots` `table/sticky-*` (bwst) laufen gelegentlich in den 15-s-Timeout — lastabhaengig, auch mit 1 Worker. Kein Pixel-Signal: einmal gezielt wiederholen (`-- --grep sticky`), gruen = Rauschen                                                                                                                                                                                              | Tabs-Skeleton-Migration (2026-09-23)               | Phantom-Regressionssuche in einer nicht beruehrten Komponente         |
...
- **Evidenz**: Base-Baselines im Docker aus `46c2d093` erzeugt, dann je Theme
  ...
  `table/sticky-cols` (Erfahrung #40), Wiederholung `bwst --check -- --grep sticky` → 4 passed,
