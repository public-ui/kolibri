...
| 34  | Der Playwright-Container erreicht die Registry nur mit dem Agent-Proxy-CA: `/root/.ccr/ca-bundle.crt` in den `docker run` mounten und `NODE_EXTRA_CA_CERTS` + `npm_config_cafile` darauf zeigen lassen, sonst bricht `npm install -g pnpm` mit `SELF_SIGNED_CERT_IN_CHAIN` ab. Lauscht der Proxy auf `127.0.0.1` (`HTTPS_PROXY`), braucht der Container zusaetzlich `--network host` und `HTTPS_PROXY`/`NO_PROXY` als `-e`                                                                                                                                                                                      | Dialog-Skeleton-Migration, TableStateless-Migration | Der einzige Blocker zwischen "kein Docker" und laufender Pipeline             |
...
| 38  | Baselines parallel zur Umsetzung erzeugen: `git worktree add ../kolibri-base origin/develop`, dort das Docker-Skript mit **eigenem Volume** (`kolibri-vt-develop`) und `--all` (Update-Modus) starten, danach die PNGs in die Snapshot-Ordner des Branches kopieren und `--all --check` fahren. Der Update-Modus laeuft nur `theme-snapshots.spec.js` (138 Tests), `--check` zusaetzlich die Axe-Snapshots (297) — ein Check kann also an Axe-Timeouts scheitern, die der Baseline-Lauf nie ausgefuehrt hat. `set -euo pipefail` bricht nach dem ersten roten Theme ab: die restlichen Themes einzeln nachholen | TableStateless-Migration                            | Kein Warten auf die Baseline, kein Arbeiten gegen einen spiegelnden Workspace |
