...
- **Lektion 2**: Auch wenn jede Computed-Property gleich ist, kann ein zusätzlicher DOM-Vorfahre über einem `position: fixed`-Compositing-Layer Firefox sub-pixel anders rastern lassen. Wenn Theme-CSS nachweislich keinen Hebel hat und der einzige Unterschied die Wrapper-Verschachtelung ist, ist der Fix ein **DOM-Fix in der Migration selbst** (hier: Tooltip aus dem `BemRootNodeFC`-Wrapper heraus, wie es develop hatte) — nicht Allowlist, nicht Theme-Arbeit.
