...
- **Offen (3): `dialog`/`drawer`/`modal` „Close"-Button-Tooltip** — ~2px vertikaler Textversatz im Tooltip-Bubble. **Voll diagnostiziert** über eine Shadow-durchdringende Probe (rekursiv alle `shadowRoot` sammeln, dann pro Root queryen — `document.querySelectorAll` bleibt an Shadow-Grenzen hängen): jede Computed-Property + Bounding-Box bit-identisch DEV vs BR (`kol-tooltip__floating`/`__arrow`/`__content` y-Positionen, Rotations-Matrix des Arrows, `font-family=Verdana`, `font-size`, `line-height=normal`, `font-kerning`, `text-rendering`, `letter-spacing`). Einziger Messunterschied: `kol-button__tooltip`-Anker-Breite 0 statt 44px (Flex-Geschwister von `__button` statt Block-Kind des `<button>`) — ohne Wirkung auf die JS-Tooltip-Lage. `&__tooltip { width: 100% }` verworfen (brach 6 Szenarien). **→ echtes Firefox-Sub-Pixel-Paint-Artefakt ohne CSS-Angriffspunkt (develop == Branch beim Tooltip-CSS); Allowlist-Kandidat §8.** Andere Themes: nicht betroffen (ihre Tooltips erben eine hinting-robustere Font statt des Verdana-a11y-Fallbacks).
- **Lektion**: Bevor „Firefox-Paint-Artefakt" als Diagnose steht, IMMER die Shadow-durchdringende Probe fahren und Element-für-Element ALLE Font- und Geometrie-Properties vergleichen — der default-`icon/font`-Fall (eine Vorsession lang als Artefakt abgetan) war in Wahrheit ein `text-align`-Bug. Erst wenn wirklich jede Property gleich ist, ist es ein Artefakt.
